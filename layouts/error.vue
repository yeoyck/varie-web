...
interface errorProp {
  ...
  description?: string
  caption?: string
  errorType?: 'info' | 'warning' | 'info'
  link?: boolean
}
const props = withDefaults(defineProps<errorProp>(), {
  title: '404',
  errorType: 'info',
  link: true
})
...
const colorPalette: { [key: string]: IColorPalette } = {
  info: {
    border: 'rgba(33, 150, 243, 0.4)',
    title: 'text-blue-500'
  },
  warning: {
    border: 'rgba(247, 149, 48, 0.4)',
  ...
  <div class="surface-0 flex align-items-center justify-content-center min-h-screen min-w-screen overflow-hidden">
    <div class="grid justify-content-center p-2 lg:p-0" style="min-width: 80%">
      <div class="col-12 mt-5 xl:mt-0 text-center">
        <img src="/logo.png" alt="Logo" class="mb-5" style="width: 80px" />
      </div>
      <div
        class="col-12 xl:col-6"
        :style="`border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg,${
          colorPalette[props.errorType].border
        } 10%, rgba(33, 150, 243, 0) 30%)`"
      >
        <div
        ...
        >
          <div class="grid flex-column align-items-center">
            <span :class="`${colorPalette[props.errorType].title} font-bold text-3xl`">{{ props.title }}</span>
            <h1 v-if="props.description" class="text-900 font-bold text-3xl lg:text-5xl mb-2">{{ props.description }}</h1>
            <span v-if="props.caption" class="text-gray-600">{{ props.caption }}</span>
            <slot></slot>
            ...
            <div v-if="props.link" class="col-12 mt-5 text-center">
